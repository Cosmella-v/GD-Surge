#include <Surge/modify/MenuLayer.hpp> #include <Geode/utils/file.hpp> #include <Geode/utils/web.hpp> #include <Geode/loader/Event.hpp> #include <Surge/layers/CreditsLayer.hpp> #include <Utils.hpp>

float speed = 1.0f; float delaySetting = 0.0f; float addtlDuration = 0.0f;

bool stopLooping = false; bool jumpedAlready = false;

bool downloadStarted = false; bool startup = false;

static auto activeListeners = std::make_shared<std::vector<std::shared_ptr<EventListenerweb::WebTask>>>();

bool MyMenuLayer::init() { if (!MenuLayer::init()) return false;
...
void MyMenuLayer::onCreator(CCObject* sender) { #ifdef GITHUB_ACTIONS FLAlertLayer::create("Creator", "This feature is <cr>disabled</c>. To <cg>enable</c> it, please disable this mod. Sorry for the inconvenience.", "OK")->show(); #else MenuLayer::onCreator(sender); #endif }

void MyMenuLayer::onStartupPopup(float dt) { auto mod = Mod::get(); auto tag = mod->getVersion().getTag(); bool isAlpha = mod->getVersion().toVString(true).find("alpha") != std::string::npos;
...
void MyMenuLayer::onTwitch(CCObject* sender) { GDSFollowLinks::onTwitch(); }

void MyMenuLayer::onTwitter(CCObject* sender) { GDSFollowLinks::onTwitter(); }

void MyMenuLayer::onDiscord(CCObject* sender) { GDSFollowLinks::onDiscord(); }

void MyMenuLayer::onFacebook(CCObject* sender) { GDSFollowLinks::onFacebook(); }

void MyMenuLayer::onRobTop(CCObject* sender) { GDSFollowLinks::onRobTop(); }

void MyMenuLayer::onYouTube(CCObject* sender) { GDSFollowLinks::onYouTube(); }
